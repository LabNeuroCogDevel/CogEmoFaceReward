...
% globals are held over from other function runs? This gets ugly.
clearvars -global
global w slack subject resppad runTotals textSize facenumC blockC emotionC rewardC ITIC experiment totalBlocks trialsPerBlock;
%window pointer, slack, and subject structure are global across functions

subject.subj_id = NaN; % we shouldn't yet know who our subject is, but we might want it defined


%% PROCESS ARGUMENTS GIVEN TO FUNCTION
% * are we using a response pad? 
% * what resolution? (default to neweyelab res)
% * what subject (or prompt if not given)
% resppad and subject are globals, screen resolution is returned

[ screenResolution ] = parseOptions(varargin{:});
...
try
    ...
    for i=startTrial:endTrial
        ...
        facetexture    = facetex.(emo){face};
        rewardfunction =  experiment{rewardC}{subject.trial_num};

        fprintf('\n\nstart trial %d (%s %d %s)\n',subject.trial_num, emo,face,rewardfunction);
        ...
        [RTms, firstClockFlip, keyPressed ] = faceWithTimer(timerDuration,facetexture,windowRect);
        ...
        % score
        [F_Mag, F_Freq] = getScore(RTms,rewardfunction);
        fprintf('RT is: %.2f => %fpts @ p =  %f)\n', RTms,F_Mag,F_Freq);
        timeshouldtake  = firstClockFlip + RTms/1000 + postResponseISI;
        % show score
        % DEF: function [scoreflip, ev, inc ] = scoreRxt(F_Mag,F_Freq, keyPressed, reftime, receiptDuration)
        [feedbackFlip,ev,inc ] = scoreRxt(F_Mag, F_Freq,keyPressed,timeshouldtake,receiptDuration);

...
##
##
## saved to FaceFMRIOrder.csv | facenum,emotion,reward
## Note that ITI distribution is randomly sampled at runtime in MATLAB
##
...
rewardFuns <- c("DEV", "IEV")

#force no repeats of emotions or reward conditions
conditionGrid <- expand.grid(emotion=emotions, reward=rewardFuns)

#manually add CEV and CEVR scrambled blocks
conditionGrid <- rbind(conditionGrid, data.frame(emotion=c("scram", "scram"), reward=c("CEV", "CEVR")))
...
write.table(blocklist,file="FaceFMRIOrder.csv",row.names=FALSE,sep=",", quote=FALSE)
